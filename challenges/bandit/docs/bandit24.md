...
??? note "Part 1 : Getting to know the daemon"

    This part will be pretty short as we already have experience with daemons
    (see [bandit14](./bandit14.md) for more explanations). We will simply connect and try to communicate
    ...
    ??? success "Solution"
        ...
        ```
        Using that information, we know the format of the string we're supposed to send to the daemon
        to try to bruteforce the password. Let's now try and use this knowledge to craft
        our brute-force attack.

??? note "Part 2 : Crafting the attack"

    Now that we know how to communicate with the server and that we noticed that indeed,
    we don't have to open a new connection for each message, let's try to generate all the
    ...
    ??? success "Solution"
        ...
        You'll be able to find a lot of solutions following the same pattern all over the internet.
        Let's try to do something a bit different.
        ...
        2. It prints the string bandit24\_pass alongside the value of the integer
        (padded with zeros on the left to fit a field width of 4 characters)

??? note "Part 3 : Launching the attack and retrieving the password"

    Now that we know what our for loop looks like, you might want to know why we used this construct
    instead of the first form. Let's not wonder about that for now and instead launch the attack.

    ??? tip "Hint"

        Using our newly constructed for loop, can you figure out a way to use
        [nc](https://www.commandlinux.com/man-page/man1/nc.1.html) to retrieve the password?
...
??? note "Part 4 : Let me think please"

    If this hasn't been patched yet, you might notice that the server blocks indefinitely after a given number
    of attempts. The goal of this last part is to ensure that the server won't block and that we'll be
    able to test all the connections.

    ??? tip "Hint"

        Using our command from the last part, would you be able to add a simple check to ensure that the server
        doesn't test all the attempts at the same time but waits a bit before sending each chunk of tests.
    ...
    ??? success "Solution"
        ...
        ```
        The `if` check ensure that the server gets time to process the input,
...
??? note "Full Solution"
    ...
    this command will test the password and all of the 10000 pin combinations agains the server pin and prints
    the password for the next level once the right pin has been entered.

    ???+ warning

        Do not forget to replace bandit24\_pass with the actual password for the bandit24 user.
