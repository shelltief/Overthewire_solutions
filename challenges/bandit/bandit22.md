...
??? note "Part 1 : Script analysis and password retrieval"
    ...
    ??? tip "Hint"
        ...
        Using the useful commands, can you figure out what the script does and in which file
        it prints the password for the next level?

    ??? success "Solution"

        Let's analyse this script.

        - The `myname` variable hold the output of the command `whoami`, which prints the username
        associated with the current user id. As this script is ran by bandit23, we know that
        the value of the myname variable is bandit23.
        - The `mytarget` variable holds the output of the following pipeline
        (where `myname` has been replaced by its value) : `echo I am user bandit23 | md5sum | cut -d ' ' -f 1`.
        ...
        Finally, the cut utility will retrieve only the first field
        (by splitting the fields using the space character), which is the hash of our phrase.

        As standard input and standard error are redirected to
        [/dev/null](https://man7.org/linux/man-pages/man4/null.4.html), we won't see the output from
        the echo command, however we can notice that the password for bandit23 is printed in
        the file `/tmp/hashed_phrase`.

        Thus, we know that by running the following command :
        ```bash
        cat /tmp/8ca319486bfbbc3663ea0fbe81326349
        ```
        (where `8ca319486bfbbc3663ea0fbe81326349` is the hash of "I am user bandit23"),
        we can retrieve the password for the bandit23 user.

??? note "Full Solution"

    1. `cat /tmp/8ca319486bfbbc3663ea0fbe81326349` to retrieve the password for the next level
